    ...
    if (!validateCsrfToken(getPostData()['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            try {
                ...
                setFlashMessage('success', 'Organisation created successfully!');
                redirect('/admin');
            } catch (Exception $e) {
                $errors['general'] = 'Failed to create organisation: ' . $e->getMessage();
            }
        }
    }
}

$title = 'Create Organisation - ' . APP_NAME;
ob_start();
?>
...
<div class="<?= DesignSystem::getCurrentSystem() === 'sgds' ? 'ds_!_margin-bottom--4' : 'mb-8' ?>">
    <header class="<?= DesignSystem::getCurrentSystem() === 'sgds' ? 'ds_page-header' : 'mb-6' ?>">
        <h1 class="<?= DesignSystem::getCurrentSystem() === 'sgds' ? 'ds_page-header__title' : 'text-3xl font-bold text-gray-900' ?>">Create New Organisation</h1>
    </header>
    ...
    <form method="POST" class="<?= DesignSystem::getCurrentSystem() === 'tailwind' ? 'max-w-4xl' : '' ?>">
        ...
        <div class="<?= DesignSystem::getCurrentSystem() === 'sgds' ? 'ds_button-group' : 'flex justify-end space-x-3' ?>">
            ...
            <?= DesignSystem::button('Create Organisation', null, 'primary', ['type' => 'submit']) ?>
        </div>
    </form>
</div>
