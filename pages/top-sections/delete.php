    ...
    if (!validateCsrfToken(getPostData()['csrf_token'] ?? '')) {
        ...
        try {
            ...
            if (!empty($section['image_path'])) {
                $fullPath = __DIR__ . '/../../' . ltrim($section['image_path'], '/');
                if (file_exists($fullPath)) {
                    @unlink($fullPath);
                }
            }
            $topSectionModel->delete($id, $organizationId);
            setFlashMessage('success', 'Top section deleted successfully!');
            redirect('/top-sections');
        } catch (Exception $e) {
...
$title = 'Delete Top Section - ' . APP_NAME;
ob_start();
?>

<div class="mb-8">
    <header class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Delete Top Section</h1>
    </header>
    ...
    <div class="bg-white shadow rounded-lg p-6 mb-6">
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-red-700">
                        <p>This action cannot be undone. The section "<strong><?= h($section['title'] ?: 'Untitled') ?></strong>" will be permanently removed from your strategic plan.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 p-4 rounded border border-gray-200 mb-4">
            ...
            <p class="text-sm text-gray-700"><strong>Type:</strong> <?= ucfirst(h($section['section_type'])) ?></p>
            <p class="text-sm text-gray-700"><strong>Title:</strong> <?= h($section['title'] ?: 'Untitled') ?></p>
        ...
        <form method="POST">
            ...
            <div class="flex justify-end space-x-3">
                <a href="/top-sections" class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                    Cancel
                </a>
