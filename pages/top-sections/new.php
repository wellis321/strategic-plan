...
if (isPost()) {
    if (!validateCsrfToken(getPostData()['csrf_token'] ?? '')) {
        ...
        // Get current max sort_order to set new section at the end
        $existingSections = $topSectionModel->getAll(['organization_id' => $organizationId]);
        $maxSortOrder = 0;
        foreach ($existingSections as $section) {
            $maxSortOrder = max($maxSortOrder, (int)$section['sort_order']);
        }
        ...
        $formData = [
            ...
            'sort_order' => (int)($postData['sort_order'] ?? ($maxSortOrder + 1)),
        ...
        if (empty($errors)) {
            try {
                $sectionId = $topSectionModel->create($formData);
                setFlashMessage('success', 'Top section created successfully!');
...
$title = 'Create Top Section - ' . APP_NAME;
ob_start();
?>

<div class="mb-8">
    <header class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Create Top Section</h1>
        <p class="mt-2 text-gray-600">
            Add a custom section that appears at the top of your strategic plan, before your goals.
        </p>
    </header>
    ...
    <form method="POST" class="max-w-4xl" enctype="multipart/form-data">
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6 space-y-6">
            <div>
                ...
                >
                    ...
                    <?php foreach ($allPlans as $plan): ?>
                        <option value="<?= $plan['id'] ?>" <?= $formData['plan_id'] == $plan['id'] ? 'selected' : '' ?>>
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Choose the type of section. Custom sections require a title.</p>
            </div>
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Use the formatting toolbar to add bold, italic, lists, and links. Press Enter to create new paragraphs.</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" for="image">Image (Optional)</label>
                <input
                    type="file"
                    id="image"
                    name="image"
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Choose where the image appears relative to the content.</p>
            </div>
            ...
            <div>
                ...
                <select
                    ...
                    class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                >
                    <option value="small" <?= $formData['size'] === 'small' ? 'selected' : '' ?>>Small (compact padding, smaller fonts)</option>
                    <option value="medium" <?= $formData['size'] === 'medium' ? 'selected' : '' ?>>Medium (standard padding, standard fonts)</option>
                    <option value="large" <?= $formData['size'] === 'large' ? 'selected' : '' ?>>Large (generous padding, larger fonts)</option>
                </select>
            ...
            <div>
                ...
                <input
                    ...
                    class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                >
                <p class="mt-1 text-sm text-gray-500">Lower numbers appear first. You can also reorder sections on the list page.</p>
            </div>
